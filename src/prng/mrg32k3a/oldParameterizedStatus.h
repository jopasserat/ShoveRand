namespace MRG32k3a {
   ...
   class ParameterizedStatus {
	...
	private: 
		typedef ::MRG32k3a::Stream Stream;
	...
	//public:
         ...
         void init(unsigned short blocksNumber) {
            ...
            allStreams_host = new Stream [blocksNumber];
      
            // allocate memory for streams on the device
            cutilSafeCall( cudaMalloc( (void**)&allStreams_, sizeof(Stream) * blocksNumber) );
            // transfer Streams 
  	...
  	// TODO once again core namespace is needed to avoid using global namespace
	typedef ::ParameterizedStatus<MRG32k3a> ParameterizedStatusMRG32k3a;
}
